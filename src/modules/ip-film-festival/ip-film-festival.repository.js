...
class IpFilmFestivalRepository {

    async create(data) {
        try {
            return await prisma.ip_international_film_festivals.create({
                data
            });
        } catch (error) {
            if (error.code === 'P2002') {
                throw new AppError('Something went wrong during film festival add.!', 409);
            }
            throw error;
        }
    }
    ...
    async updateByFormId(id, data) {
        ...
        const form = await prisma.ip_international_film_festivals.findUnique({
            where: {
                id: BigInt(id),
            }
        });
        ...
        return await prisma.ip_international_film_festivals.update({
            where: {
                id: BigInt(id)
            },
            data
    ...
    async findByFormId(id) {
        return await prisma.ip_international_film_festivals.findFirst({
            where: {
                id
            }
        });
    ...
    async delete(id) {
        const deleted = await prisma.ip_international_film_festivals.delete({
            where: { id }
        });
        if (!deleted) throw new AppError('Ip film festival not found.', 404);
        return deleted;
    }
}

export default new IpFilmFestivalRepository();